            ...
            for i in 1:grid_size
                for j in 1:grid_size
                    if j < i
                        ...
                        x_data_emu = emu_out.population[3][:,i]
                        y_data_emu = emu_out.population[3][:,j]
                        x_data_sim = sim_out.population[3][:,i]
                        y_data_sim = sim_out.population[3][:,j]
                        ...
                        contourf(contour_x, contour_y, contour_z, 8,
                                    colors=["white", "#FFE9EC", "#FFBBC5", "#FF8B9C", "#FF5D75", "#FF2F4E", "#D0001F", "#A20018"],
                                    zorder=1)
...
function plot_distances_training_accepted(emu_out::ABCSMCOutput)
    for smc_iteration in 1:length(emu_out.threshold_schedule)
        for parameter_idx in 1:emu_out.n_params
            subplot_idx = (smc_iteration - 1) * 3 + parameter_idx
...
f = figure()
ioff()
plot_distances_training_accepted(emu_out_4_6)
subplots_adjust(
left    =  0.08,
bottom  =  0.06,
right   =  0.96,
